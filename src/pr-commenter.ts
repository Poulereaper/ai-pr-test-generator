  ...
  if (userCommand) {
    ...
    try {
      ...
      if(!options.aiLightModeluses && TokenPrompt > options.aimaxtokens) {
        ...
        postAIResponse(aiResponse ?? '', userCommand, options)
...
export async function postAIResponse(
  ...
  try {
    ...
    if (!github_context.issue?.number) {
      warning('No issue number found in context, cannot post comment')
      return
    }

    // Prepare the response body with formatting
    let responseBody = `🤖 **AI Analysis Complete**\n\n`
    
    // Add command context if available
    if (userCommand) {
      responseBody += `**Command**: \`${userCommand.action} ${userCommand.filename || ''}\`\n`
      if (userCommand.customPrompt) {
        responseBody += `**Custom instructions**: ${userCommand.customPrompt}\n`
      }
      responseBody += `\n---\n\n`
    ...
    // Add footer
    responseBody += `\n\n---\n*Generated by AI Test Generator Bot*`
    ...
    if (options?.debug) {
      info('Successfully posted AI response as comment')
    }
    
  } catch (error) {
    console.error('Error posting AI response:', error)
    
    // Try to post an error message instead
    try {
      await octokit.rest.issues.createComment({
        owner: github_context.repo.owner,
        repo: github_context.repo.repo,
        issue_number: github_context.issue.number,
        body: `❌ **Error**: Failed to post AI response.\n\n**Error details**: ${error instanceof Error ? error.message : 'Unknown error'}`
      })
    } catch (secondaryError) {
      console.error('Failed to post error message:', secondaryError)
      warning(`Failed to post AI response and error message: ${secondaryError}`)
    }
  }
}